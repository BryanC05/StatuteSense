...
function setItems(key, items) {
  if (typeof window === 'undefined') return { success: false, error: 'No window environment' };
  try {
    localStorage.setItem(key, JSON.stringify(items));
    return { success: true };
  } catch (error) {
    console.error('localStorage quota error or other error:', error);
...
export function saveDocument(doc) {
  ...
  docs.unshift(newDoc);
  const result = setItems(STORAGE_KEYS.documents, docs);
  if (result && !result.success) return { error: result.error };
  return newDoc;
}
...
export function saveAnalysis(analysis) {
  ...
  const result = setItems(STORAGE_KEYS.analyses, analyses);
  if (result && !result.success) return { error: result.error };
  return newAnalysis;
}
...
export function getDeadlines() {
  const docs = getDocuments().slice(0, 50); // limit to recent 50 docs
